begin;
  ...
  begin
    ...
    insert into credential_vault_ssh_cert_library
      (store_id,       public_id,      name,                    description, vault_path,         username, key_type, key_bits, extensions)
    values
      ('vs______swvs', 'vscl____wvl1', 'widget ssh admin cert', 'None',      '/ssh/issue/admin', 'admin',  'rsa',    4096,     '{"permit-pty": ""}'),
      ('vs______swvs', 'vscl____wvl2', 'widget ssh user cert',  'None',      '/ssh/sign/user',   'user',   'ecdsa',  521,      '{"permit-pty": "", "permit-X11-forwarding": ""}');
